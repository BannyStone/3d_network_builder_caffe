...
from modules import BNReLUModule, BNReLUConvModule, TemporalConvModule
from blocks import PreActWiderDecoupBlock, PreActWiderTempConvBlock
...
def write_prototxt(is_train, output_folder, \
    ...
    if is_train:
        data_train_params = dict(name='data', \
                            ...
                            transform_param=dict(
                                ...
                                mean_value=[104]*16+[117]*16+[123]*16), \
    ...
    #### Stage 1 ####
    channels = 3*7*7*3*num_output_stage1/(7*7*3+3*num_output_stage1)
    ...
    stage1 = TemporalConvModule(name_template='1', \
                                bn_params=bn_params, \
                                stride=2, \
                                num_output=num_output_stage1).attach(netspec, [conv1_1xdxd])
    ...
    for stage in range(4):
        for block in range(blocks[stage]):
            ...
            last = PreActWiderTempConvBlock(name_template=name, \
                                        shortcut=shortcut, \
                                        num_output=curr_num_output,
                                        stride=stride).attach(netspec, [last])

    #### Last Norm & ReLU ####
    last = BNReLUModule(name_template='5b', \
                        bn_params=bn_params).attach(netspec, [last])
