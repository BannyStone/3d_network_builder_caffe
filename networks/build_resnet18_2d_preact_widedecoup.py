...
from blocks import PreActWiderDecoupBlock, PreActWiderDecoup2dBlock
...
def write_prototxt(is_train, output_folder, \
    ...
    if is_train:
        data_train_params = dict(name='data', \
                            ...
                            image_data_param=dict( \
                                source="../imagenet_train_list.txt", \
                                batch_size=32, \
                                new_width=width, \
                                new_height=height, \
                                shuffle=True), \
        ...
        # pdb.set_trace()
        netspec.data, netspec.label = BaseModule('ImageData', data_train_params).attach(netspec, [])
        netspec.vdata, netspec.vlabel = BaseModule('ImageData', data_val_params).attach(netspec, [])
    else:
        data_params = dict(name='data', \
                            dummy_data_param=dict( \
                                shape=dict(\
                                    dim=[10, 3, crop_size, crop_size])))
    ...
    for stage in range(4):
        for block in range(blocks[stage]):
            ...
            last = PreActWiderDecoup2dBlock(name_template=name, \
                                        shortcut=shortcut, \
